...
/**
 * Main activity class for the Mjolnir control panel application. Provides UI controls and logic for
...
public class ControlPanelActivity extends Activity
{
    ...
    /**
     * Constructor. Creates a new ControlPanelActivity object.
     */
	...
	private void disconnect()
	{
		...
		// Terminate robot subsystem connections.
		mMjolnir.StopNavSubsystem();
		mMjolnir.StopVideoSubsystem();
		
		// Clear the video frame image.
	...
	private class VideoFrameReceivedListener implements VideoConnection.VideoFrameReceivedListener
	{
		...
		private void drawDebugOverlay(Bitmap bm)
		{
			...
			String[] text = mMjolnir.getNavigationHistory();
		...
		public void onVideoFrameReceived(final Bitmap bm)
		{
			...
			ControlPanelActivity.this.runOnUiThread(new Runnable()
			{
				public void run()
				{
					if (connected)
